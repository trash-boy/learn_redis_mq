...
func (c *Client)XACK(ctx context.Context, topic, groupID, msgID string)error{
	if topic == "" || groupID == "" || msgID == ""{
		return errors.New("redis XACK tpoic| group_id|msg_id can not be empty")
	}
	conn,err := c.pool.GetContext(ctx)
	if err != nil{
		return err
...
func (c *Client)Get(ctx context.Context, key string)(string,error){
	if key == ""{
		return "",errors.New("redis GET key can not be empty")
	}
	conn,err := c.pool.GetContext(ctx)
	if err != nil{
		return "",err
...
func (c *Client)Set(ctx context.Context, key,val string)(int64,error){
	if key == "" || val == ""{
		return -1,errors.New("redis Set key can not be empty")
	}
	conn,err := c.pool.GetContext(ctx)
	if err != nil{
		return -1,err
...
func (c *Client)SetNEX(ctx context.Context, key,val string, expireSeconds int64)(int64,error){
	if key == "" || val == ""{
		return -1,errors.New("redis SetNEX key can not be empty")
	}
	conn,err := c.pool.GetContext(ctx)
	if err != nil{
		return -1,err
	...
	resp,err := conn.Do("SET", key, val,"EX", expireSeconds)
	if err != nil{
		return -1,err
	}
	if respStr,ok := resp.(string); ok && strings.ToLower(respStr) == "ok"{
...
func (c *Client)SetNX(ctx context.Context, key,val string)(int64,error){
	...
	resp,err := conn.Do("SET", key, val, "NX")
	if err != nil{
		return -1,err
	}
	if respStr,ok := resp.(string); ok && strings.ToLower(respStr) == "ok"{
